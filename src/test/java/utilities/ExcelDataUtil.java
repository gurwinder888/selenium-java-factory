...
//        init(testDatafilePath, sheetName);
        ...
        if (!found) LogUtil.infoLog(ExcelDataUtil.class, "No data found with given key-> " + testCaseID);
    ...
    public static CommonSettings getCommonSettings() {
        ...
        try (FileInputStream fis = new FileInputStream(System.getProperty("user.dir") +
                ConfigReader.getValue(automationcontrolexcelpath)); Workbook wb = WorkbookFactory.create(fis)) {
    ...
    public static boolean isSuiteRunnable(String suiteName) {
        ...
        try {
            ...
            isRunnable = strVal.equalsIgnoreCase(Y);
        } catch (Exception e) {
            isRunnable = false;
            LogUtil.errorLog(ExcelDataUtil.class, EXCEPTIONCAUGHT, e);
        }
